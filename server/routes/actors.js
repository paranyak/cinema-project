...
router.get("/autocomplete/:query", async (req, res) => {
  ...
  const actors = await db
    .get()
    .collection("actors")
    .find(params, { fields: { id: true, name: true, movies: true, slugName: true } })
    .toArray()
...
router.post("/", async (req, res) => {
  ...
  actor.movies.forEach(async movie => {
    await db
      .get()
      .collection("movies")
      .findOneAndUpdate({ slugName: movie }, { $push: { cast: actor.slugName } })
  })
