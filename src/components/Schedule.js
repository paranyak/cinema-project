...
class Schedule extends Component {
  render() {
    ...
    unfetched.filter(slug => !this.props.isMovieFetching(slug)).forEach(slug => this.props.fetchMovieBySlug(slug))
    ...
    return (
      <section className={b()}>
        ...
        <main className={b("film-list")}>
          <time
            className={b("real-time-line")}
            style={{
              left:
                this.globalScale(Interval.fromDateTimes(sessionStart, DateTime.local()).toDuration().milliseconds) +
                "%",
              display: sessionStart.hasSame(DateTime.local(), "day") && films.length > 0 ? "block" : "none",
            }}
          />
          <span className={b("films-placeholder")} style={{ display: films.length === 0 ? "block" : "none" }}>
          ...
          {films.map((film, i) => (
            <article key={"div-1lev" + i.toString()} className={b("film")}>
              ...
              <section
                style={{ minHeight: film.schedule.length * 10 }}
                key={"div-2lev" + i.toString()}
                  ...
                  <div key={i}>
                    <time
                      ...
                      style={{
                        ...
                        left: this.scale(Interval.fromDateTimes(sessionStart, s.start).toDuration().milliseconds) + "%",
                      }}
                    />
                    <ReactTooltip />
                  </div>
...
export default connect((state, props) => {
  ...
  const date = filters.date ? DateTime.fromFormat(filters.date, "yyyy-MM-dd") : DateTime.local()
  return {
    date,
    unfetchedMovies,
