...
class EditActorPage extends Component {
  ...
  getStateFromChild(keys, values) {
    ...
    for (let k = 0; k < keys.length; k++) {
      ...
      if (
        ...
        (values[k].length === 0 || Object.values(values[k]).includes(NaN))
      ) {
        canSubmit = false
      }
    }
  ...
  async editActorInDB(e) {
    e.preventDefault()
    const { info, date, city, nominations, image, name, movies, oldMovies } = this.state
    const { actor } = this.props

    let newMovies = movies
    if (movies.length !== 0 && typeof movies[0] === "object") {
      newMovies = movies.map(m => m.slugName).filter(slugName => slugName.trim() !== "")
      movies.filter(el => el.slugName.trim() !== "").forEach(el => {
  ...
  render() {
    ...
    const { fireRedirect, info, city, name } = this.state
    ...
    const isEnabled = name.length * cInfo.length * cCity.length !== 0
